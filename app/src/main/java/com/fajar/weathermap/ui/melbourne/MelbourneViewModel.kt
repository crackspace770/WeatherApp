...
class MelbourneViewModel(private val context: Context) : ViewModel() {
    ...
    companion object {
        private const val TAG = "MelbourneViewModel"
        private const val MELBOURNE_DATA_KEY = "melbourne_data"
    }

    init {
        fetchWeatherData(28.0836, -80.6081)
    }
    ...
    private fun saveWeatherData(weatherResponse: WeatherResponse?) {
        weatherResponse?.let {
            ...
            editor.putString(MELBOURNE_DATA_KEY, json)
            editor.apply()
        }
    }

    private fun loadWeatherData() {
        val json = sharedPreferences.getString(MELBOURNE_DATA_KEY, null)
        json?.let {
            val gson = Gson()
